...
//    private final static Logger logger = LoggerFactory.getLogger(RandomUtil.class);
    ...
    /**
     * 根据int[][]里的权重随机出值 每个数组中的最后一值是权重
     *
     * @param intArr 要随机的数组,权重
     * @return 返回值为去掉权重后的数组
     */
    public static int[] randomIntArrayByWeight(int[][] intArr) {
        int total = 0;
        for (int[] arr : intArr) {
            int len = arr.length;
            total += arr[len - 1];
        ...
        int[] attr = intArr[index];
        int realLen = attr.length -1;
        int[] targetAttr = new int[realLen];
        System.arraycopy(attr, 0, targetAttr, 0, realLen);
        return targetAttr;
    }
